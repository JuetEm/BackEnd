...
# kakao-firebase 로그인 API
import kakaoLogin 
from kakaoLogin import *
# 네이버 검색 API 예제 
...
# 프로젝트의 모든 사용자 나열
# Start listing users from the geginning. 1000 at a time
tester_1th =[]
index = 0
...
while page:
    for user in page.users:
        ...
        userObj['user'] = "user_"+str(index)
        print("User : "+user.email)
        userObj['email'] = user.email
        print("User : "+str(user.user_metadata.creation_timestamp))
        print("User : "+str(user.user_metadata.last_refresh_timestamp))

        # fromtimestamp() 함수는 milliseconds 가 아니라 seconds를 인자로 받음
        creationTime = user.user_metadata.creation_timestamp/1000
        formatedCreationTime = datetime.fromtimestamp(creationTime).strftime("%Y, %m, %d")
        print("formatedCreationTime : {}".format(formatedCreationTime))
        userObj['registerDate'] = formatedCreationTime

        if user.user_metadata.last_refresh_timestamp is not None:
            lastLogintime = user.user_metadata.last_refresh_timestamp/1000
            formatedlastLogintime = datetime.fromtimestamp(lastLogintime).strftime("%Y, %m, %d")
            print("formatedlastLogintime : {}".format(formatedlastLogintime))
            userObj['lastLogIn'] = formatedlastLogintime
        else:
            lastLogintime = ''

        tester_1th.append(userObj)
...
""" for user in auth.list_users().iterate_all():
    ...
    # print("user.metadata.creationTime: "+user.creationTime)
...
def kakaoSearch():
    ...
    size = flask.request.args.get('size',15)
    page = flask.request.args.get('page',1)
    print("query : {}".format(query))

    locales = KakaoSearchLocale(query,size,page)
...
""" @app.route("/kakaoMapForFlutter", methods=["GET"])
def kakaoMapForFlutter():

    return render_template('basicMapForFlutter.html') """

@app.route("/kakaoMapForFlutter", methods=["GET","POST"])
def kakaoMapForFlutter_post():

    resultData = request.form.get('queryString')
    print("resultData : {}".format(resultData))

    return render_template('basicMapForFlutter.html', flutterApiData = resultData)

@app.route("/auth/kakaoLogin", methods=["GET","POST"])
def kakaoLogin():
    print("/auth/kakaoLogin 울린다!!")
    ...
    print("uid : {}, displayName : {}, email : {}, photoURL : {}".format(uid,displayName,email,photoURL))

    customToken = createCustomtoken(uid,displayName,email,photoURL)
